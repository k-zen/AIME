...
public abstract class AbstractFetchSchedule extends Configured implements FetchSchedule
{
    ...
    private static final String KEY = AbstractFetchSchedule.class.getName();
    ...
    public void setConf(Configuration conf)
    {
        ...
        {
            LOG.debug("Fetch Interval: " + defaultInterval);
            LOG.debug("Max. Fetch Interval: " + maxInterval);
        }
    }

    /**
     * Initialize fetch schedule related data. Implementations should at least
     * set the <code>fetchTime</code> and <code>fetchInterval</code>. The default implementation sets the
     * <code>fetchTime</code> to now, using the default <code>fetchInterval</code>.
     *
     ...
     * implementation resets the retry counter - extending classes should call super.setFetchSchedule() to preserve this behavior.
     */
    @Override
    ...
    /**
     * This method specifies how to schedule re-fetching of pages marked as
     * GONE. Default implementation increases fetchInterval by 50%, and if it
     * exceeds the <code>maxInterval</code> it calls {@link #forceRefetch(Text, CrawlDatum, boolean)}.
     *
     * @param url              URL of the page
     ...
     * due to transient errors. The default implementation sets the next fetch
     * time 1 day in the future and increases the retry counter.
     *
     * @param url              URL of the page
     ...
     * selection in the current fetch list. NOTE: a true return value does not
     * guarantee that the page will be fetched, it just allows it to be included
     * in the further selection process based on scores. The default
     ...
     * retriesSinceFetch and page signature, so that it forces re-fetching.
     *
     * @param url   URL of the page
     * @param datum datum instance
